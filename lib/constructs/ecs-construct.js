...
class ECSConstruct extends Construct {
    constructor(scope, id) {
        ...
        // FundingRate Service
        const fundingRateTaskDef = new ecs.FargateTaskDefinition(this, 'fundingRateTaskDef', {
            cpu: 256,
            memoryLimitMiB: 512,
        });
        fundingRateTaskDef.addContainer('fundingRate', {
            image: ecs.ContainerImage.fromRegistry(process.env.ECR_IMAGE_URI_MAIN),
            command: ['node', 'marketsSync/fundingRates/syncFundingRate'],
            logging: ecs.LogDrivers.awsLogs({
                streamPrefix: 'syncFundingRate',
                logGroup: log(this, "fundingRateLogGroup"),
            }),
            environment: {
                NODE_ENV: 'production',
                REDIS_URI: process.env.REDIS_URI || '',
        ...
        executionRolePolicy(fundingRateTaskDef);
        this.fundingRateService = new ecs.FargateService(this, 'fundingRateService', {
            cluster: this.cluster,
            taskDefinition: fundingRateTaskDef,
            desiredCount: 1,
        });
    }
}
